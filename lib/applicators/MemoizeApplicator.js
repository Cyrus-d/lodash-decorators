...
var MemoizeApplicator = /** @class */ (function (_super) {
    ...
    MemoizeApplicator.prototype.apply = function (_a) {
        ...
        var resolver = utils_1.resolveFunction(lodash_es_1.isFunction(args[0]) ? args[0] : lodash_es_1.isObject(args[0]) ? args[0].resolver : args[0], instance, target, false);
        if (resolver && instance) {
            resolver = resolver.bind(instance);
        }
        var memoized = resolver ? execute(value, resolver) : execute(value);
        if (lodash_es_1.isObject(args[0])) {
            var _b = args[0], cache = _b.cache, type = _b.type;
            if (cache) {
                memoized.cache = cache;
            }
            else if (lodash_es_1.isFunction(type)) {
